#!/usr/bin/env python3
"""
Standalone Stage 2 IPP pretraining.

Trains IPP only, using frozen ContextEncoder (required) and optional
frozen SurprisePredictor for CE surprise features.
"""
...
def _context_and_target_from_batch(
...
def train_epoch(
    ...
    for batch in loader:
        v_context, target_vecs = _context_and_target_from_batch(
        ...
        if v_context is None or target_vecs is None:
            continue
        ...
        nn.utils.clip_grad_norm_(ipp.parameters(), clip_grad)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Stage2 IPP-only pretraining")
    parser.add_argument("--config", type=str, required=True, help="JSON config path")
    parser.add_argument("--resume", type=str, default=None, help="Resume checkpoint")
    parser.add_argument("--device", type=str, default=None, help="Device override")
    parser.add_argument("--ce-checkpoint", type=str, default=None, help="Override CE checkpoint path")
    ...
    if not ce_ckpt_path:
        raise ValueError("IPP pretraining requires CE checkpoint path (--ce-checkpoint or init.ce_checkpoint).")

    print(f"Loading frozen CE from {ce_ckpt_path}")
    ...
    if use_surprise:
        ...
        if not sp_ckpt_path:
            raise ValueError("IPP pretraining requires SP checkpoint when use_surprise_features=true")
        ...
        print("SP features disabled for IPP pretraining.")
    ...
    if ipp_mode == "flow":
        ...
        if sigma_init > 0.1:
            print(
                ...
                f"Flow may show high train cos but near-random eval samples."
            )

    lr = float(train_cfg.get("lr", train_cfg.get("ipp_lr", 1e-4)))
    ...
    if weight_decay > 0.1:
        print(
            ...
            f"this can cap representation quality."
        )

    num_epochs = int(train_cfg.get("num_epochs", train_cfg.get("ipp_num_epochs", 20)))
    ...
    checkpoint_dir = Path(out_cfg.get("checkpoint_dir", "experiments/08_autoregressor/ipp/checkpoints"))
    logs_dir = Path(out_cfg.get("logs_dir", "experiments/08_autoregressor/ipp/logs"))
    ...
    log_path = logs_dir / "ipp_training_log.jsonl"
    ...
    print(f"\n{'=' * 60}")
    print(f"IPP-only pretraining ({num_epochs} epochs)")
    print(f"{'=' * 60}")
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        print(f"[IPP] Epoch {epoch} done ({elapsed:.1f}s)")
        ...
        if do_eval:
            ...
            print(
                f"  [IPP VAL] loss={val_metrics.get(loss_key, 0.0):.4f} "
    ...
    log_file.close()
    print(f"\n{'=' * 60}")
    print("IPP pretraining complete")
