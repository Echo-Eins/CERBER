...
def create_step_metrics_figure(run: dict[str, Any] | None) -> go.Figure:
    ...
    _add_line_if_present(fig, df, "train_step", "train_loss_aux", "train aux loss", "#f783ac", secondary_y=False)
    _add_line_if_present(fig, df, "train_step", "train_loss_aux_df", "train aux DF loss", "#91a7ff", secondary_y=False)
    _add_line_if_present(fig, df, "val_epoch", "val_loss_aux", "val aux loss", "#faa2c1", secondary_y=False)
    _add_line_if_present(fig, df, "val_epoch", "val_loss_aux_df", "val aux DF loss", "#bac8ff", secondary_y=False)
    ...
    _add_line_if_present(fig, df, "train_step", "train_aux_cos_mean", "train aux cos", "#ff6bcb", secondary_y=True)
    _add_line_if_present(fig, df, "train_step", "train_aux_cos_answer", "train aux answer cos", "#f06595", secondary_y=True)
    _add_line_if_present(fig, df, "train_step", "train_aux_df_cos_mean", "train aux DF cos", "#5c7cfa", secondary_y=True)
    ...
    _add_line_if_present(fig, df, "val_epoch", "val_aux_cos", "val aux cos", "#d6336c", secondary_y=True)
    _add_line_if_present(fig, df, "val_epoch", "val_aux_cos_answer", "val aux answer cos", "#e64980", secondary_y=True)
    _add_line_if_present(fig, df, "val_epoch", "val_aux_df_cos", "val aux DF cos", "#4263eb", secondary_y=True)
